...
internal sealed class EventsContainerAccessor(
    IPawfrontSecretProvider secretProvider,
    IOptions<CosmosOptions> cosmosOptions) : IEventsContainerAccessor, IAsyncDisposable
{
    ...
    public async Task<Container> GetContainerAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(options.Containers.Events))
            {
                throw new InvalidOperationException("Cosmos:Containers:Events is required.");
            }
            ...
            container = client.GetContainer(options.DatabaseName, options.Containers.Events);
