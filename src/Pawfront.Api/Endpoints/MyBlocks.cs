...
/// of leaving the provider to wonder why they can no longer message the customer
/// on it.
/// </para>
/// <para>
...
{
    public static Task<MyBlockedCounterparties> ForProviderAsync(
        Guid providerId,
        IMyBlockLookup lookup,
        CancellationToken cancellationToken)
        => providerId == Guid.Empty
            ? Task.FromResult(MyBlockedCounterparties.Empty)
            : lookup.GetAsync(new BlockParty(BlockPartyType.Provider, providerId), cancellationToken);
