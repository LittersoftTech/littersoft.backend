...
internal static class EventEndpoints
{
    public static IEndpointRouteBuilder MapEventEndpoints(this IEndpointRouteBuilder builder)
    {
        ...
        providerScoped.MapPut("/{eventId:guid}", Update);
        providerScoped.MapPatch("/{eventId:guid}", Patch);
    ...
    /// <summary>
    /// Partial edit. Reads the current event, overlays only the fields present
    /// in the body (each is an <see cref="Optional{T}"/>), then runs the same
